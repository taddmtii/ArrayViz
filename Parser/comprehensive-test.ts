...
function printState(state: State, label: string) {
  console.log(`\n--- ${label} ---`);
  console.log(
    `Evaluation Stack: [${state.evaluationStack.map((v) => serializeValue(v)).join(", ")}]`,
  );
  console.log(
    `Variables:`,
...
function executeAndShow(
  ...
  commands.forEach((cmd, idx) => {
    ...
    cmd.do(state);
    console.log(
      `  Stack after: [${state.evaluationStack.map((v) => serializeValue(v)).join(", ")}]`,
    );
...
console.log(
  "█" + "  COMPREHENSIVE EXPRESSION AND COMMAND TEST SUITE".padEnd(68) + "█",
);
console.log("█" + " ".repeat(68) + "█");
...
const floatLiteral = new NumberLiteralExpressionNode(
  "3.14",
  createToken("3.14"),
);
...
state = createFreshState();
const binaryLiteral = new NumberLiteralExpressionNode(
  "0b1010",
  createToken("0b1010"),
);
...
state = createFreshState();
const stringLiteral = new StringLiteralExpressionNode(
  createToken('"Hello World"'),
);
executeAndShow(
...
state = createFreshState();
const falseLiteral = new BooleanLiteralExpressionNode(
  false,
  createToken("False"),
);
...
executeAndShow(
  stringAccess.evaluate(),
  state,
  'String Access: mystr[1] where mystr="Hello"',
);
...
executeAndShow(
  complex2.evaluate(),
  state,
  "Comparison with Arithmetic: (10 + 5) > 10",
);

...
function printState(state, label) {
    ...
    console.log("Variables:", Array.from(state.variables.entries()).map(function (_a) {
        var k = _a[0], v = _a[1];
...
function executeAndShow(commands, state, description) {
    ...
    commands.forEach(function (cmd, idx) {
        ...
        if (state.variables.size > 0) {
            console.log("  Variables: ".concat(Array.from(state.variables.entries()).map(function (_a) {
                var k = _a[0], v = _a[1];
                return "".concat(k, "=").concat(serializeValue(v));
            }).join(', ')));
        }

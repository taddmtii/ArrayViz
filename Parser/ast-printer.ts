...
class ASTVisualizer {
  ...
  visualizeCompact(node: any, depth: number = 0): void {
    ...
    const props = Object.entries(node).filter(
      ([key]) => !["_startTok", "_endTok", "_tok"].includes(key),
    );
  ...
  visualizeTypes(node: any, prefix: string = "", isLast: boolean = true): void {
    ...
    if (typeof node !== "object") {
      console.log(
        `${prefix}${this.getConnector(isLast)}${typeof node}: ${String(node).substring(0, 20)}`,
      );
      return;
    }
    ...
    if (node._value !== undefined) {
      console.log(
        `${prefix}${this.getConnector(isLast)}${nodeType} (${node._value})`,
      );
      return;
    }
